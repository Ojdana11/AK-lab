...
_start:
	...
	mov $ZERO, %edx
	pop %edx # wynik funkcji dodaj w %edx	
	mov %edx, %eax
	call pokaz_wynik
...
silnia:
  	...
  	mov 16(%ebp), %ebx # pobranie 1 parametru, czyli 5
...
dodaj:
  	push %ebp #zapis stosu programu głównego 
  	mov %esp, %ebp # ustawienie nowej ramki miejscu akt. poz. stosu 
	mov 16(%ebp), %eax # pobranie 1 parametru, czyli 10
	mov 24(%ebp), %ebx # pobranie 2 parametru, czyli 5
	
	#proste obliczenia, wynik w rbx
	add %eax, %ebx
	mov %ebx, 16(%ebp) #zwrocenie wyniku w miejsce 10
